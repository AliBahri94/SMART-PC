...
class Group_3(nn.Module):
    ...
    def forward(self, xyz):
        ...
        batch_size, num_points, _ = xyz.shape
        # fps the centers out
        center = pytorch3d.ops.sample_farthest_points(xyz, K= self.num_group, random_start_point= True)[0]  
        # center = misc.fps(xyz, self.num_group)  # B G 3    sample 128 center points from 2048 points
        # knn to get the neighborhood
        # _, idx = self.knn(xyz, center)  # B G M,   kNN samples for each center  idx (B, M, G)   every center has G (group size) NN points
...
class MaskTransformer_SMART(nn.Module):   
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
        # define the transformer argparse
...
class MaskTransformer_SMART_WITH_MASK(nn.Module):   
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
        # define the transformer argparse
...
class Point_MAE(nn.Module):
    def __init__(self, config):
        ...
        elif (self.method != "MATE" and self.dataset == "shapenetcore"):
            self.class_head = nn.Sequential(*class_blocks, nn.Linear(last_dim, self.cls_dim))

        elif (self.method != "MATE" and self.dataset == "modelnet"):
            self.class_head = nn.Sequential(*class_blocks, nn.Linear(last_dim, self.cls_dim))   
        ...
        if (self.normalize == "minus_center"):
            if (self.repeat_data_SMART_PC  == False):  
                self.group_divider = Group(num_group=self.num_group, group_size=self.group_size)
            else:
                self.group_divider = Group_3(num_group=self.num_group, group_size=self.group_size)

        else:
            if (self.repeat_data_SMART_PC  == False):
                self.group_divider = Group_2(num_group=self.num_group, group_size=self.group_size)
            else:
    ...
    def compute_loss_mean(self, shape_xyz, skel_xyz, skel_radius, A, w1, w2, w3=0, lap_reg=False):
        bn = skel_xyz.size()[0]
        shape_pnum = float(shape_xyz.size()[1])
        skel_pnum = float(skel_xyz.size()[1])
        B = float(skel_xyz.size()[0])
        ...
        cd_sample1 = DF.closest_distance_with_batch(sample_xyz, shape_xyz) / (skel_pnum * 8 * B)    
        cd_sample2 = DF.closest_distance_with_batch(shape_xyz, sample_xyz) / (shape_pnum * B)
        loss_sample = cd_sample1 + cd_sample2

        # point2sphere loss
        skel_xyzr = torch.cat((skel_xyz, skel_radius), 2)
        cd_point2pshere1 = DF.point2sphere_distance_with_batch(shape_xyz, skel_xyzr) / (shape_pnum * B)
        cd_point2sphere2 = DF.sphere2point_distance_with_batch(skel_xyzr, shape_xyz) / (skel_pnum * B)
        loss_point2sphere = cd_point2pshere1 + cd_point2sphere2

        # radius loss
        loss_radius = - torch.sum(skel_radius) / (skel_pnum * B)
        ...
        # loss combination
        final_loss = loss_sample + loss_point2sphere * w1 + loss_radius * w2 + loss_smooth * w3
        # final_loss = (0 * loss_sample) + loss_point2sphere * w1 + loss_radius * w2 + loss_smooth * w3   
    ...
    def classification_SMART(self, pts, only_unmasked=True):
        B, _, _ = pts.shape  # B VIS C 
        neighborhood, center = self.group_divider(pts)
        x_vis_w_token = self.MAE_encoder(neighborhood, center, only_unmasked=only_unmasked)[0]
        x_full = x_vis_w_token[:, 1:]
        pos_full = self.decoder_pos_embed(center).reshape(B, -1, 384)
        x_rec = self.MAE_decoder(x_full, pos_full, 0)
        guid = x_rec + x_vis_w_token[:, 1:]
        feat = torch.cat([x_vis_w_token[:, 0], guid.max(1)[0]], dim=-1) 
        class_ret = self.class_head(feat)             
        return class_ret       

    def classification_SMART_2(self, pts, only_unmasked=True):
        B, _, _ = pts.shape  # B VIS C 
        neighborhood, center = self.group_divider(pts)
        x_vis_w_token = self.MAE_encoder(neighborhood, center, only_unmasked=only_unmasked)[0]
        x_full = x_vis_w_token[:, 1:]
    ...
    def forward(self, pts, vis=False, cyclic=False, **kwargs):
        ...
        if (self.method == "MATE"):
            ...
            loss1 = self.compute_loss(pts, skel_xyz, skel_radius[..., None], None, 0.3, 0.4)  
...
class PointTransformer(nn.Module):
 ...
 ################################# plot gussian bnimport numpy as np
...
# plot_bn_gaussians(running_mean, running_var, save_path='bn_layer1_stats.png')
