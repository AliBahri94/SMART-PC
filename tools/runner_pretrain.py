...
def load_base_model(args, config, logger, load_part_seg=False):
    ...
    base_model.load_model_from_ckpt(args.ckpts_MATE, load_part_seg)
    if args.use_gpu:
    ...
    if not args.partnet_cls:
        ...
        for epoch in range(start_epoch, config.max_epoch + 1):
            ...
            for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
                ...
                if (config.model.transformer_config.aug == "scale_transform"):
                    points = train_transforms(points)
                ...
                if idx % 20 == 0:
                    if (method != "SMART_PC_N_MATE"):
                        ...
                        print_log(
                            '[Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.3f (s) Losses (total/rec/skl/class) = %s, Acc = %s lr = %.6f' %
                            (epoch, config.max_epoch, idx + 1, n_batches, batch_time.val(), data_time.val(),
                            ['%.4f' % l for l in losses.val()], acc.item(), optimizer.param_groups[0]['lr']),
                            logger=logger)
...
def validate(base_model, extra_train_dataloader, test_dataloader, epoch, val_writer, args, config, logger=None, method = None):
    if not args.partnet_cls:
        ...
        with torch.no_grad():
            for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
                ...
                # elif (method == "SMART_PC_N" or method == "SMART_PC_P"):    
                    ...
                    ret = base_model.module.classification_SMART_2(points, only_unmasked=False)      
